    ...
    def redraw_plot(self):
        ...
        for folder_name in self.data_order:
            if self.visibility_vars[folder_name].get():  # 只绘制可见的系列
                ...
                scaled_y_data = y_data * scale_factor
                ...
                # 应用Smooth化
                x_smooth, y_smooth = self.apply_single_smoothing(x_data, scaled_y_data)
                
                # 绘制数据
                if self.global_smoothing_var.get():
                    # 绘制原始数据点（较淡）
                    self.ax.scatter(x_data, scaled_y_data, s=point_size, alpha=0.4, color=color, edgecolors='none', label=f"{legend_name} (原始)")
                    ...
                    # 原始散点图
                    self.ax.scatter(x_data, scaled_y_data, label=legend_name, s=point_size, alpha=0.7, color=color)
        ...
        handles, labels = self.ax.get_legend_handles_labels()
        if handles:  # 如果有图例元素
            self.ax.legend(loc=self.label_position_var.get(), prop={'size': legend_size})
...
class MultiVariablePlotController:
    ...
    def redraw_plot(self):
        ...
        for var in self.variables:
            ...
            for dataset_index, (folder_name, df) in enumerate(self.data_list):
                ...
                # 应用Smooth化
                x_smooth, y_smooth = self.apply_smoothing(x_data, y_data, var)
                ...
                if self.smoothing_states[var].get():
                    ...
                    # 原始数据点（较淡）
                    scatter_orig = current_ax.scatter(x_data, y_data, s=point_size*0.6, alpha=alpha*0.4, 
                                                    color=color, edgecolors='none', label=f"{label} (原始)")
                    ...
                    # 只绘制散点图
                    scatter = current_ax.scatter(x_data, y_data, s=point_size, alpha=alpha, 
        ...
        if self.dual_y_enabled.get() and self.ax2:
            ...
            if left_lines:
                legend1 = self.ax1.legend(left_lines, left_labels, loc=left_legend_pos, 
                                        prop={'size': legend_fontsize, 'family': 'Times New Roman'})
    ...
    def apply_axis_limits(self):
        ...
        try:
            ...
            if x_min_str.lower() != 'auto' and x_max_str.lower() != 'auto':
                try:
            ...
            if self.dual_y_enabled.get() and self.ax2:
                ...
                if left_y_min_str.lower() != 'auto' and left_y_max_str.lower() != 'auto':
                    try:
                        y_min = float(left_y_min_str)
                        y_max = float(left_y_max_str)
                        if y_min < y_max:
                            self.ax1.set_ylim(y_min, y_max)
                    except ValueError:
                        pass
                elif left_y_min_str.lower() != 'auto':
                    try:
                        y_min = float(left_y_min_str)
                        ...
                        pass
                elif left_y_max_str.lower() != 'auto':
                    try:
                        y_max = float(left_y_max_str)
                        current_ylim = self.ax1.get_ylim()
                ...
                if right_y_min_str.lower() != 'auto' and right_y_max_str.lower() != 'auto':
                    try:
                        y_min = float(right_y_min_str)
                        y_max = float(right_y_max_str)
                        if y_min < y_max:
                            self.ax2.set_ylim(y_min, y_max)
                    except ValueError:
                        pass
                elif right_y_min_str.lower() != 'auto':
                    try:
                        y_min = float(right_y_min_str)
                        ...
                        pass
                elif right_y_max_str.lower() != 'auto':
                    try:
                        y_max = float(right_y_max_str)
                        current_ylim = self.ax2.get_ylim()
                ...
                if y_min_str.lower() != 'auto' and y_max_str.lower() != 'auto':
                    try:
